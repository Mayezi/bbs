    ...
    var tpl = [
      ...
      <li>\
        {{# if(d.rows[i].collection_time){ }}\
          <a class="topic-title" href="/topic/{{d.rows[i].id}}.html" target="_blank">{{= d.rows[i].title}}</a>\
        ...
        {{# } else { }}\
          ...
          <span class="topic-status topic-status-ok">已解决</span>\
          {{# } }}\
          <a class="topic-title" href="/topic/{{d.rows[i].id}}.html" target="_blank">{{= d.rows[i].title}}</a>\
          <i>{{new Date(d.rows[i].time).toLocaleString()}}</i>\
          {{# if(d.rows[i].accept == -1){ }}\
          <a class="mine-edit" href="/topic/edit/{{d.rows[i].id}}">编辑</a>\
          {{# } }}\
      ...
      if(gather.minelog[type + '-page-' + curr]){
        ...
        if(type === 'collection'){
          ...
          fly.json('/api/'+ type +'/', {
            ...
            view(res);
            gather.minelog['mine-topic-page-' + curr] = res;
            now || laypage({
              cont: 'LAY_page'
              ,pages: res.pages
              ,skin: 'fly'
